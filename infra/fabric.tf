...
# ---------------------------------------------------------------------
# Nightly auto-pause limits idle Fabric-capacity cost. Resume remains a
# manual step (`fabric/manage_capacity.py resume`) because demo timing is
# irregular. The schedule uses UTC because Terraform's
...
# ---------------------------------------------------------------------
# Shared Fabric Data Agent -- grounded in the Eventhouse for live
# Factory/Quality telemetry. ERP/Supply Chain context is deliberately
# routed through the Fabric IQ Ontology rather than duplicated into the
# same agent.
# ---------------------------------------------------------------------
