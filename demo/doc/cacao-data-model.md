...
10 lines total. See `demo/data-generation/src/seed_data.py`.
...
production stage. See `demo/ontology/farm-preparation.md`.
...
In `ontology_config.json` shape (`demo/ontology/ontology_config.json`).
...
never edited in place. All chocolate-scenario code is authored in `demo/`
at the project root instead. Infra/deployment layers in both upstream
repos are reusable as-is once this content is deployed into them.

| Area | Status | Notes |
|---|---|---|
| `demo/ontology` | **Config done** | `ontology_config.json` — all 18 tables, 3 domains. Supply Chain / ERP dimension CSVs, `scenarios.json` registration, and 4-agent provisioning in `fabric-ontology` still to do |
| `demo/data-generation` | **Built, verified against a live Event Hub** | Streams `sensor_reading`, `quality_check`, `batch_event`, `line_status`. Supply Chain / ERP generators not started. Dashboard tiles not started |
| `demo/eventhouse` | **KQL + Eventstream definition written, not yet run against a live Eventhouse** | Bronze/Silver/Gold + `eventstream.json` — see `cacao-medallion-layers.md` |
| `demo/agents` | **Not started** | Coordinator + 3 specialist instruction sets, orchestration/handoff logic, chat API routing |
