...
`demo/kb/*.md` is automated by `terraform apply` (see
[`README.md`](README.md) and `demo/infra/README.md`'s "Foundry IQ
...
- **A Foundry project** — specifically a *project-based* Foundry
  ...
  separate Azure resource from anything `demo/infra` provisions, and
...
- The Search service and the Fabric workspace must be **in the same
  ...
  tenant by `demo/infra`.

## Values you'll need

Pull these from `demo/infra` (`terraform output`, run from
`demo/infra/`):
...
one `demo/kb/` doc, and confirm it answers correctly and cites that
...
This should return `4` (one per `demo/kb/00`–`03` doc). If it doesn't,
the problem is upstream of the knowledge base — re-run
`terraform apply` in `demo/infra`, or manually re-run the indexer
...
`demo/kb/README.md`'s "Deploying" section and the FabCon demo plan's
