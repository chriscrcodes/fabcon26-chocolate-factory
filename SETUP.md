...
Fresh deployments have three additional prerequisites:

- **Workspace and connection display names are tenant-wide unique.**
  If `WorkspaceNameAlreadyExists` or `DuplicateConnectionName` occurs,
  use a unique suffix across the whole tenant.
- **The Fabric Data Agent must be published before querying.** This is
  handled by `null_resource.publish_data_agent` and
  `fabric/data-agent/publish_data_agent.py`.
- **OneLake shortcuts may briefly be unavailable after mirroring is
  enabled.** `fabric/ontology/deploy_onelake_shortcuts.py` retries with
  backoff to absorb that propagation delay.
...
**Project choice: Eventhouse-only optimization.** The shared Data Agent
is intentionally bound to the Eventhouse's `silver_quality_check` and
`silver_line_status` tables via `type: "kusto"`. Live telemetry stays
here; ERP/Supply Chain context is handled by the Fabric IQ Ontology.

`silver_batch` is excluded because it behaves like a materialized view,
not a plain table.

**This is a repo design decision, not a universal Fabric rule.** The
Lakehouse-backed ERP/Supply Chain path was tested and did not work in
this scenario, but the demo keeps the shared Data Agent focused on the
Eventhouse and uses the Ontology for cross-domain business context.
...
The Supply Chain and ERP/Orders domains have both an ontology schema
(`fabric/ontology/ontology_config.json`) and seeded business data.
`simulator/src/business_data.py` generates the ten business tables,
and `run_business_seed.py` loads them into the Fabric SQL Database.
The KB files describe the business semantics used to ground questions
over that live seeded data.
...
- **❌ Known gap** — either run live and failed, or established as a
  deployed data-model limitation. Useful for the "known gaps" beat in
  [`README.md`](README.md#known-limitations), not for the main
  script.

Summary: **12** verified, **2** candidates, **5** known gaps.
The verified results were run on 31 Aug 2026 against a freshly
refreshed GraphModel with live telemetry flowing. Candidate questions
were not run live; known gaps include both observed failures and
limitations established from the deployed data model.
...
#### Known gaps — useful for the honesty beat, not the main script
