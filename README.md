...
The simulator starts at the factory boundary: cocoa nibs are the
already fermented, dried, roasted and winnowed output of upstream
origin processing. The six stages below are the production line that
the demo models and streams.

```mermaid
flowchart LR
  subgraph ORIGIN["Origin processing · upstream of the factories"]
    HARVEST["Harvest & ferment"] --> DRY["Drying"] --> ROAST["Roasting"] --> WINNOW["Winnowing"]
  end

  NIBS["Cocoa nibs"]

  subgraph FACTORY["Chocolate factory · six simulated stages"]
    GRIND["Grinding"] --> MIX["Mixing & refining"] --> CONCH["Conching"]
    CONCH --> TEMP["Tempering"] --> MOLD["Molding & cooling"] --> PACK["Packaging"]
  end
...
all its tools. After apply, refresh the Ontology Graph Model once in
the Fabric portal so its static bindings are available. Full deploy
prerequisites, the simulator (real-time, backfill, and per-scenario
commands), and the query walkthrough are all in **[`SETUP.md`](SETUP.md)**.
...
### Design choice: shared Data Agent vs. Ontology

The shared Fabric Data Agent is intentionally Eventhouse-only as a
project optimization: it stays focused on live factory telemetry and
quality signals, while ERP/Supply Chain context is routed through the
Fabric IQ Ontology instead of being duplicated into the same agent.

This is a design choice for the demo, not a claim that Fabric Data
Agents are universally limited to Eventhouse-only sources.

**A2A orchestration — intentionally not used in the demo.** We tested
it, but the product path for live A2A wiring was not stable enough for
this build. The simpler pattern used here is one orchestrating agent
reasoning across multiple specialized tools and knowledge sources.
...
| | Count | Meaning |
|---|---|---|
| ✅ Verified | 12 | Run live, exact result recorded — safe to use as-is |
| 🧪 Candidate | 2 | Plausible given the real data model, not yet run live |
| ❌ Known gap | 5 | Tested failure or documented data-model limitation |
...
- The Fabric Data Agent is intentionally Eventhouse-only: Fabric exposes
  no `sql_database` source type, and the tested Lakehouse alternative also
  failed, including the portal-generated configuration (see
  [`SETUP.md`'s `fabric/data-agent` section](SETUP.md#fabricdata-agent)).
  Supply Chain/ERP grounding therefore uses the Fabric IQ Ontology.
