...
are bound here. The binding schema imposes two constraints:

- `EventhouseTableDataBindingProperties` (`sourceType: KustoTable`) is
    accepted only with `dataBindingType: TimeSeries`; other entity types
    use `LakehouseTable`.
- A TimeSeries entity's identifying key must be a static `properties`
    entry with its own static data binding. `STATIC_BINDING_SOURCE` adds
    that second `NonTimeSeries` binding; without it, GraphModel refresh
    fails with `GraphNotRefreshable`.
