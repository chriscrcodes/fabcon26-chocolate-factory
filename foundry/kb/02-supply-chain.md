...
> `fabric/ontology/README.md`'s "Not yet built").
...
point, not necessarily another factory). Status is expected to progress
through a lifecycle such as: pending → in transit → delivered (exact
status vocabulary to be finalized when the generator is built). Shipment
data is what connects a factory's production output to the ERP/Orders
domain's fulfillment picture — a question like "will this order ship on
time" spans both domains and belongs to the Coordinator.
