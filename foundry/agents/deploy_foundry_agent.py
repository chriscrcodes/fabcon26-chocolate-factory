...
def build_definition() -> dict:
    ...
    return {
        ...
        "instructions": (
            ...
            "that same factory's quality checks). Remote tool calls on "
            "this platform execute strictly one after another even "
            "when requested concurrently, so each additional call adds "
            "real latency -- when a question has two or more sub-parts "
            "that are ALL relationship/structural questions answerable "
            "by fabric_iq_ontology alone (entities, properties, "
            "relationships -- never quality-check or line-status "
            "counts, which fabric_iq_ontology cannot see), fold them "
            "into a single fabric_iq_ontology call with one compound "
            "natural-language query instead of one call per sub-part. "
            "Never fold a fabric_data_agent-only sub-part (quality "
            "checks, line status) into a fabric_iq_ontology query -- "
            "call fabric_data_agent for it directly, phrased the same "
            "way you would ask a person (e.g. 'how many quality checks "
            "failed today at the factories receiving shipments today'), "
            "chained after fabric_iq_ontology only when it must resolve "
